...
viewTeam.init = function (teamId, u) {
    ...
    $('#createMember').on('click', function () {
        var fields = [
            { id: 'fullName', label: 'Meno a priezvisko', type: 'text', required: 1 },
            {
                ...
                type: 'date',
                locales: viewTeam.user.locales,
                dateFormat: viewTeam.user.dateFormat,
            },
    ...
    $('#btnFounderDetails').on('click', function (event) {
        var fields = [
            {
                id: 'btnCpyFromBill',
                label: 'Kopíruj údaje z fakturačných',
                type: 'button',
                onclick: function () {
                    viewTeam.cpyAdr('B', 'F', teamId);
                },
            },
            {
                id: 'foundingOrg.name',
            ...
            {
                id: 'foundingAdr.addrLine1',
                label: 'Adresa - riadok 1',
                type: 'text',
                placeholder: 'adresa riadok 1',
                required: 1,
            },
            {
                id: 'foundingAdr.addrLine2',
                label: 'Adresa - riadok 2',
                type: 'text',
                placeholder: 'adresa riadok 2',
            },
            {
                id: 'foundingAdr.city',
                label: 'Mesto',
    ...
    $('#btnBillingDetails').on('click', function (event) {
        var fields = [
            {
                id: 'btnCpyFromFnd',
                label: 'Kopíruj údaje zo zriaďovateľa',
                type: 'button',
                onclick: function () {
                    viewTeam.cpyAdr('F', 'B', teamId);
                },
            },
            {
                id: 'billingOrg.name',
    ...
    $('#btnAddCoach').on('click', function (ev) {
    ...
    $('#btnRegister2').on('click', function (event) {
        var fields = [
            {
                ...
                init: function (domid, cb) {
                    libCommon.loadList(domid, '/program?cmd=getList&active=1&teamId=' + teamId, cb);
                },
                onchange: function () {
                    libCommon.loadList(
                        'MFDFavailEvents',
                        '/event?cmd=getList&active=1&program=' + $('#MFDFavailProgs').val()
                    );
                },
            },
            {
            ...
            {
                id: 'acceptRules',
                label: 'Súhlasím s podmienkami programu a turnaja',
                type: 'checkbox',
                required: true,
            },
        ];
        ...
        libModals.multiFieldDialog(
            ...
            fields,
            function (flds, cb) {
                viewTeam.registerForEvent(
                    teamId,
                    flds.find(function (f) {
                        return f.id === 'availEvents';
                    }).value,
                    cb
                );
            },
            ...
            if (res.list.length > 0) {
                ...
                res.list.forEach(function (item) {
                    if (item.fullName) {
                        ...
                        g.append(c);
                        c = $(
                            '<button id="RMC' + item._id + '" class="btn btn-success coach-remove">'
                        ...
                        var c = $('<div class="panel panel-default card">')
                            ...
                            .append(
                                $('<div class="panel-body form-inline">')
                                    .append(
                                        $(
                                            '<input class="form-control" type="string" readonly value="' +
                                                (item.dateOfBirth
    ...
    $.post(
        ...
        function (res) {
            ...
            if (res.result == 'ok') {
                ...
                if (res.messages) {
                    res.messages.forEach(function (m, i) {
                        alert(m.message);
                    });
                }
    ...
    $.get(libCommon.getNoCache('/invoice?cmd=getList&teamId=' + teamId), function (res) {
        ...
        if (res.result === 'ok') {
            ...
            res.list.sort(function (a, b) {
                return a.issuedOn > b.issuedOn ? 1 : b.issuedOn > a.issuedOn ? -1 : 0;
            });
            sel.empty();
            selu.empty();
            if (res.list.length > 0) {
                ...
                res.list.forEach(function (item) {
                    var c = $('<li class="list-group-item">').append(
                        $('<h5 class="list-group-item-heading">').append(
                    ...
                    if (!item.isDraft)
                        c.append(
                            $('<p class="list-group-item-text">')
                                ...
                                .append(
                                    item.paidOn
                                        ? '  Zaplatená ' + moment(item.paidOn).format('L')
                                        : '  Splatná ' +
                                              (item.dueOn ? moment(item.dueOn).format('L') : '')
                                )
                        );
                    ...
                    if (item.type == 'P' && !item.taxInvoice && !item.isDraft)
                        c.append(
                            $(
                                '<button id="CIN' +
                                    item._id +
                                    '" class="btn btn-default createTaxInvoice">'
            ...
            if (res.list.length > 0) {
                ...
                res.list.forEach(function (item) {
                    if (item.name.length > 0) {
                        var c = $(
                            '<a class="list-group-item" href="/docs?cmd=download&doc=' +
                                item.key +
                                '"">'
                        ).append(
                            i++ + '. ' + item.name + ' [' + Math.round(item.size / 1024) + ' kiB]'
                        );
                        sel.append(c);
                    }
                });
