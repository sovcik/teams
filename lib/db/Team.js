...
exp.getTeamDetails = function(myId, teamId) {
    return new Promise(async function (fulfill, reject) {
        ...
        try {
            ...
            // apply permissions
            const te = await TeamEvent.findOne({teamId:t.id, programId:pr.id});
            if (te) {
                t.registeredOn = te.registeredOn;
                const ev = await Event.findOneActive({_id:te.eventId});
                if (ev)
                    t.eventName = ev.name;
                else
                    t.eventName = "-- undefined ---";
            }
            switch (p){
                case 'P1':
                    t.billingContact = {};
