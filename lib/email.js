...
exp.sendProfileChangedNotification = function(userOld, user, url) {
    try {
        ...
        log.ERROR(
            'Error sending profile change notification. user=' +
                userOld.username +
                ' err=' +
                err.message
        );
...
exp.sendPwdResetCode = function(user, ot, url) {
    try {
        ...
        log.ERROR(
            'Error sending password reset code. user=' + user.username + ' err=' + err.message
        );
    }
};
...
exp.sendEventRegisterConfirmation = function(user, team, event, url) {
    ...
    try {
        ...
        log.ERROR(
            'Error sending event registration. team=' +
                team.name +
                '  event=' +
                event.name +
                ' err=' +
...
exp.sendInvoice = async function(user, invoice, url) {
    ...
    try {
        if (!invoice.team.name) log.WARN('sendInvoice: invoice.team is not populated.');
        ...
        else log.WARN('No email for invoice issuing contact specified. invoice=' + invoice.number);

        if (invoice.billContact && invoice.billContact.email) toEml.add(invoice.billContact.email);
        else log.WARN('No email for invoice billing contact specified. invoice=' + invoice.number);
        ...
        log.ERROR(
            'Error sending invoice ' + invoice.number + ' to ' + user.email + ' err=' + err.message
        );
    }
};
...
exp.sendInvoicePaid = function(user, invoice, url) {
    ...
    try {
        if (!invoice.team.name) log.WARN('sendInvoicePaid: invoice.team is not populated.');
        ...
        log.ERROR(
            'Error sending payment confirmation for invoice ' +
                invoice.number +
                ' to ' +
                user.email +
                ' err=' +
    ...
    for (let eml of toList) {
        (function(opts) {
            transporter.sendMail(opts, function(error, info) {
                if (error) {
                    log.WARN(
                        'EMAIL FAILED ' + debugPrefix + ': to=' + opts.to + ' err=' + error.message
                    );
                } else log.INFO('EMAIL SENT ' + debugPrefix + ': to=' + opts.to + ' id=' + info.messageId + ' response=' + info.response);
            });
