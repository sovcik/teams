    ...
    return new Promise(
        async function(fulfill, reject){
            ...
            try {
                ...
                if (t)
                    isCoach = true; // user does exist in role coach for specified team, so he is coach

            } catch (err) {
                log.ERROR("Error checking if user is coach err="+err.message);
    ...
    return new Promise(
        async function(fulfill, reject){
            ...
            try {
                ...
                log.ERROR("Error checking if user is InvOrgManager err="+err.message);
    ...
    return new Promise(
        async function(fulfill, reject){
            ...
            try {
                ...
                log.ERROR("Error checking if user is EventManager err="+err.message);
    ...
    return new Promise(
        async function(fulfill, reject){
            ...
            try {
                ...
                if (teamId) {
                    p.isCoach = await libPerm.isCoach(req.user.id, teamId);
                }

                if (invOrgId){
                    p.isInvoicingOrgManager = await libPerm.isInvOrgManager(req.user.id, invOrgId);
                }

                if (eventId) {
                    p.isEventManager = await libPerm.isEventManager(req.user.id, eventId);
                }

            } catch (err) {
                log.ERROR("Error getting user permissions err="+err.message);
                return reject(p);
            }
            return fulfill(p);
