...
exp.createInvoice = function(invOrgId, invType, billOrg, billAdr, billContact){
    return new Promise(
        async function (fulfill, reject) {
            console.log("CREATE INVOICE DRAFT");

            try {
                const io = await InvoicingOrg.findOneAndUpdate({ _id: invOrgId }, { $inc: {nextDraftInvNumber:1}});
                ...
                // todo: load items from specified tempolate
                inv.items = [];
                inv.items.push({itemNo:1, text:"FLL Challenge Set 2018/2019", unit:"ks", qty:1, unitPrice:129.0, total:129.0});
                inv.items.push({itemNo:2, text:"FLL Registračný poplatok 2018/2019", unit:"ks", qty:1, unitPrice:137.0,  total:137.0});
...
exp.createTeamEventInvoice = function(teamId, eventId, invType){
    return new Promise(
        async function (fulfill, reject) {
            console.log("CREATE TEAM EVENT INVOICE");
            ...
            try {
                ...
                if (!e) reject(new Error("event not found id="+eventId ));

                let i = await exp.createInvoice(e.invoicingOrg, invType, t.billingOrg, t.billingAdr, t.billingContact);
                ...
                console.log("CREATE TEAM EVENT INVOICE FAILED error=",err);
                reject(err);
            }
    ...
    return new Promise(
        async function (fulfill, reject) {
            ...
            try {
                ...
                let i = await exp.createInvoice(invOrgId, invType, t.billingOrg, t.billingAdr, t.billingContact);
    ...
    return new Promise(
        async function (fulfill, reject) {
            ...
            try {
                ...
                inv.items = [];
                inv.items.push({itemNo:1, text:"Template line 1", unit:"pcs", qty:1, unitPrice:1.0, total:1.0});
                inv.items.push({itemNo:2, text:"Template line 2", unit:"pcs", qty:2, unitPrice:2.0,  total:2.0});
