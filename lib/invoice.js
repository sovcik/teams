    ...
    return new Promise(
        async function (fulfill, reject) {
            ...
            try {
                ...
                inv.issuedOn = Date.now();
                inv.dueOn = inv.issuedOn + io.dueDays;
                inv.type = invType;
                ...
                inv.items.push({itemNo:2, text:"Zľava z registračného poplatku FLL 2017/2018", note:"Vďaka sponzorom FLL Slovensko o.z.", unit:"ks", qty:1, unitPrice:-107.0, total:-107.0});

                inv.currency = "€";
                inv.items.each(it => inv.total=inv.total+it.total);
    ...
    return new Promise(
        async function (fulfill, reject) {
            ...
            try {
                ...
                inv.billingOrg = oldInv.billingOrg;
                inv.billingAdr = oldInv.billingAdr;
                inv.billingContact = oldInv.billingContact;
                inv.issuedOn = Date.now();
                inv.dueOn = inv.issuedOn + io.dueDays;
                inv.type = invType;
