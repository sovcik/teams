...
const debugLib = require('debug')('lib-invoice');
const logERR = require('debug')('ERROR:lib-invoice');
const logWARN = require('debug')('WARN:lib-invoice');
const logINFO = require('debug')('INFO:lib-invoice');
...
exp.createInvoice = function (invOrgId, invType, billOrg, billAdr, billContact, templateId) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('createInvoice');
        debug('CREATE INVOICE DRAFT');
        ...
        try {
            ...
            debug('CREATE INVOICE DRAFT FAILED error=%s', err);
            reject(err);
        }
    });
};
...
exp.createTeamInvoice = function (invOrgId, invType, teamId) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('createTeamInv');
        debug('CREATE TEAM INVOICE');
        ...
        try {
            ...
            debug('CREATE TEAM INVOICE FAILED error=%s', err);
            reject(err);
        }
    });
};
...
exp.confirmInvoice = function (invId) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('confirmInvoice');
        debug('CONFIRM INVOICE id=' + invId);
        ...
        try {
            ...
            debug('CONFIRM INVOICE DRAFT FAILED error=%s', err);
            reject(err);
        }
    });
};
...
exp.copyInvoice = function (invoiceId, newInvoiceType) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('copyInvoice');
        debug('COPYING INVOICE');
        ...
        try {
            ...
            if (newInvoiceType === 'I') {
                try {
                    ...
                    logERR(
                        'Failed writing tax invoice number %s to original invoice %s',
                        i.id,
                        oldInv.id
                    );
                }
            ...
            debug('COPY INVOICE FAILED error=%O', err);
            reject(err);
        }
    });
};
...
exp.removeInvoice = function (invoiceId) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('removeInvoice');
        debug('REMOVING INVOICE');
        ...
        try {
            ...
            debug('REMOVE INVOICE FAILED error=%s', err);
            reject(err);
        }
    });
};

exp.createTemplateInvoice = function (invOrgId, name) {
    return new Promise(async function (fulfill, reject) {
        const debug = debugLib.extend('createTemplInv');
        debug('CREATING INVOICE TEMPLATE name=%s', name);

        try {
            let inv = Invoice();
            inv.type = 'T'; // creating new invoice template
            inv.number = 'Template ' + (name || '');
            inv.name = name;
            inv.isDraft = false;
            ...
            debug('CREATING INVOICE TEMPLATE FAILED error=%s', err);
            reject(err);
        }
    });
};
