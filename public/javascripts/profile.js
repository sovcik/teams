...
viewProfile.loadMyOrgs = function() {
    ...
    if (null === document.getElementById('myOrgsData'))
    ...
    if (selTeamName.val().trim() != '' && selTeamName.val().trim().length >= 5) {
        ...
        $.post('/team/', { cmd: 'create', name: selTeamName.val(), coach: coachId }, function(res) {
            ...
            if (res.result == 'ok') {
                ...
                } else viewProfile.loadCoachOfTeams(coachId);
                selTeamName.val('');
            } else {
...
viewProfile.changePassword = function(userId) {
    ...
    if (!validatePassword(selNewPwd.val())) {
        alert(
            'Nové heslo musí obsahovať aspoň jedno číslo, malé písmeno a musí mať aspoň 6 znakov.'
        );
        return;
    }
